...
`/pr:deslop` skills (`plugins/rebase/skills/rebase/SKILL.md` § Context):
...
Per `plugins/commit/skills/commit/SKILL.md` § Rules:
...
- `plugins/pr/skills/deslop/SKILL.md` — sibling command for
...
- `plugins/pr/skills/review-pr/SKILL.md` § Evaluate Against Quality
  Patterns — "No brittle details" rubric; the underlying source for
  several Tier B signatures.
- `plugins/pr/skills/review-pr/SKILL.md` § Rules — "Never modify the PR —
  only report findings" posture; mirrored here as
  `/slop:scan`'s never-rewrite-history rule.
- `plugins/rebase/skills/rebase/SKILL.md` § Context — trunk detection
...
- `plugins/commit/skills/commit/SKILL.md` § Rules — forbids `git add -A`,
