...
description: >-
  ...
  with insertion counts, line numbers, and links that have rotted. Fetches the
  live item, keeps provenance that cannot be re-derived, relocates depth into
  a document instead of deleting it, converts a done-checklist into invariants
  plus non-binding intent, shows a diff, and rewrites in place only on
  approval. Operates on one item at a time.
user-invocable: true
...
Then offer, via `AskUserQuestion`: update the item, update and also create the
