...
- GitHub Project (delivery_group) — native, optional; items are `grouped_by` it.
  Spans repositories, holds draft issues, caps at 50 fields ([projects][proj]).
...
`--project` and `--type`, so a drafted body goes in from a file undamaged by
shell quoting.
...
Issue and attaching it to a Project. Sub-issue relations are REST, not CLI flags
— `POST /repos/{owner}/{repo}/issues/{issue_number}/sub_issues`, plus
`GET .../sub_issues`, `GET .../parent`, `DELETE .../sub_issue` and
`PATCH .../sub_issues/priority` ([sub-issues API][subapi]) — so reach them with
`gh api`. GitHub also maintains an official MCP server, remote at
...
"Project" — write GitHub Project (delivery_group), account-owned and not a
Repository; classic Projects are a separate retired thing. Never say "board" as
