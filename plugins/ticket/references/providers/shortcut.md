...
- **Story Relationship (`StoryLink`)** — `blocks`, `duplicates`, `relates_to`.
  native, optional.
...
- **Epic Health / Objective Health** — checkpoint. native, optional. A dated
  status (On Track, At Risk, Off Track) with a comment, kept as health history.
