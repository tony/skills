---
description: Survey disk usage across every filesystem layer and classify what is reclaimable, without deleting anything
...
Read `${CLAUDE_PLUGIN_ROOT}/references/virtual-disks.md` first.
...
`${CLAUDE_PLUGIN_ROOT}/references/candidate-catalog.md`. Classify at
...
`${CLAUDE_PLUGIN_ROOT}/references/agent-history.md`. Identify each
...
in `${CLAUDE_PLUGIN_ROOT}/references/redundancy-proofs.md` and record
