---
name: disk
description: Use when a machine is low on disk space, or the user wants to find what is consuming it and clean it up. Triggers on phrases like "disk is full", "out of space", "what is eating my disk", "where did my space go", "clean up my drive", "find the biggest directories", "reclaim disk space", "clean up crap", "my home directory is huge", "analyze disk usage", or "free up space". Surveys every filesystem layer, classifies each candidate as regenerable cache, proved-redundant copy, or irreplaceable agent history, and reclaims only what a proof says is safe. Protects LLM transcripts and session stores by default. Never halts a virtual machine or WSL guest on its own initiative.
...
`/disk:usage` runs the survey and classification read-only.
`/disk:reclaim` runs the full pipeline through to execution.
