...
description: >-
  Use when the user wants to take a ticket — or several tickets that share
  one change — into its own branch and git worktree and drive it to gated
  commits. Triggers on phrases like "work TEA-123 in a worktree", "spin up a
  worktree for issue #45", "start on this ticket", "branch for the Linear
  ticket", "put these two tickets on one branch", or "prep a worktree, I'll
  take it from there". Resolves the ticket strictly read-only (never
...
on the *current* branch, use the `respond-action` skill; this command starts
ticket work on a *new* branch.

the `action-worktrees` skill delegates to this file's phases by name and
...
non-interactive run (CI, subagent — including the `action-worktrees` skill
...
2. **Directory collision** — the computed path exists on disk but is
   *not* a worktree of this branch (unregistered directory, or
   registered to a different branch) — halt with `ask-user-choice`:
...
7. End with an `ask-user-choice` panel (skip when already in plan
