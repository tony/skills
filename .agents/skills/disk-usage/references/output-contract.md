...
the `disk-reclaim` skill adds two sections after the above.
...
already inside plan mode. After this skill, that is proceeding to
the `disk-reclaim` skill. After the `disk-reclaim` skill, it is the handoff steps or a
