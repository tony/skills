...
If `ask-user-choice` is unavailable (headless mode via `claude -p`), use `variant_count` value if set, otherwise default to 1 variant. Timeout uses `timeout_value` if set, otherwise the command's default timeout.
