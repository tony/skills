...
How the `spike-probe` skill, the `spike-bakeoff` skill, the `spike-ratchet` skill,
the `respond-action` skill, the `respond-goal` skill, the `action-worktree` skill, and
this skill learn, at runtime, which verification the project
