...
description: >-
  ...
  three fixed variants that sweep across DX Bliss (frictionless), DX
  Serenity (calm clarity), and DX Sublimity (showcase-grade novelty).
...
the `weave-brainstorm-and-refine` skill instead.
...
- `--judge=*` — serene-bliss locks `--judge=panel`; users who want
  host or round-robin should run the `weave-brainstorm-and-refine` skill.
...
> serene-bliss locks all three. Run the `weave-brainstorm-and-refine` skill
...
the `weave-brainstorm-and-refine` skill verbatim — read
...
the `weave-brainstorm-and-refine` skill verbatim — assemble
...
the `weave-brainstorm-and-refine` skill — flag parsing,
...
the `weave-brainstorm-and-refine` skill verbatim. Present all
successful brainstorm originals
to the user, ask which ones enter refinement via `ask-user-choice`,
...
the `weave-refine` skill — scoring rubric (4 dimensions ×
...
the `weave-brainstorm-and-refine` skill verbatim, with one
...
Detection) of the `weave-brainstorm-and-refine` skill: if
