...
name: spike-loop
description: >-
  Use when one spike will not settle a design and the work needs repeated
  contact with real code — probe, bake off the approaches the probe's
  stumbling blocks put in doubt, graft the runners-up, probe again, and stop
  when nothing new fights back. Triggers on phrases like "rinse and repeat",
  "spike then bakeoff then spike again", "keep spiking until it's right",
  "iterate until the design settles", "converge on an approach", "prove it
  out then rewrite it from scratch", or a port or rewrite whose shape is not
  yet known. Keeps a ledger of stumbling blocks and locked decisions outside
...
metadata:
  ...
  source: "plugins/spike/skills/loop/SKILL.md"
---

# this skill

Convergence harness. the `spike-probe` skill answers "does this path work?".
the `spike-bakeoff` skill answers "which path is best?". Neither answers "what
...
approach is already clear, use the `spike-probe` skill. When two or three named
approaches compete once, use the `spike-bakeoff` skill.
...
Inherited whole from the `spike-probe` skill, including its rationalization
...
- **Stumbling blocks** — what fought back, per the `spike-probe` skill's
...
the `spike-probe` skill — typed goal wins, empty mines the conversation,
...
the `spike-probe` skill Phase 0 once for the whole loop — conventions files, the
